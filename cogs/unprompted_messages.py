...
class UnpromptedMessages(commands.Cog, name="Unprompted Messages"):
    ...
    async def on_message(self, message):
        if message.guild and message.guild.id in funcs.readJson("data/unprompted_messages.json")["servers"] \
            ...
            if lowercase.startswith("im ") or lowercase.startswith("i'm ") or lowercase.startswith("i‘m ") \
                    or lowercase.startswith("i’m ") or lowercase.startswith("i am "):
                if lowercase.startswith("im "):
                    im = originalmsg[3:]
                elif lowercase.startswith("i’m ") or lowercase.startswith("i'm ") or lowercase.startswith("i‘m "):
                    im = originalmsg[4:]
                else:
                    im = originalmsg[5:]
    ...
    async def umenable(self, ctx):
        ...
        if ctx.guild.id not in serverList:
            ...
            return await ctx.send("`Enabled unprompted messages for your server.`")
    ...
    async def umdisable(self, ctx):
        ...
        if ctx.guild.id in serverList:
            ...
            data["servers"] = serverList
            funcs.dumpJson("data/unprompted_messages.json", data)
            return await ctx.send("`Disabled unprompted messages for your server.`")
